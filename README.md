...
To put these values on context, the baseline model took 16.443 mins to train for 10 epochs on the whole data and got Accuracy: 4.6%, Top 3 Accuracy 10.8% whereas our model took 251 sec ~4.2 mins to train and had Accuracy: 18.5%, Top 3 Accuracy 33.9%. Also, please keep in mind that these accuracies are low because we are only doing 10 epochs and this as done to expedite hyperparameter tuning and training since having to run for hundreds of epochs everytime we change some parameter would take a few hours for a single model.
