...
As can be surmised from the above plot, a CNN outperforms an RNN by a significant margins. However, this differnce is probably significantly exaggerated here. It is possible and even highly likely that the CNN model we found could have already been optimized for increased performance. However, we believe that even if we had found equally simple RNN and CNN models, that the CNN model would have an a shorter training time and higher accuracy. It would be interesting to see how an LSTM+CNN model would perform given that the lack of convolutions in our final model is probably what makes the CNN significantly better.
